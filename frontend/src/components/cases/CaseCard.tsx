...
export function CaseCard({ caseData, searchQuery = '' }: CaseCardProps) {
  ...
  const { data: hearingsData = [] } = useQuery({
    ...
    queryFn: () => api.getHearings({ caseId: caseData.id }),
  });
  ...
  return (
    <div
      className="card-elevated p-4 cursor-pointer group transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1 hover:shadow-2xl relative overflow-hidden"
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-mono font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded transition-colors group-hover:bg-primary/10 group-hover:text-primary">
              {caseData.reference}
            </span>
          ...
          <div className="flex items-center gap-1 mt-2 text-sm text-muted-foreground">
            <Users className="h-3.5 w-3.5 flex-shrink-0 group-hover:text-primary/70 transition-colors" />
          ...
          <div className="flex items-center gap-1 mt-1 text-sm text-muted-foreground">
            <MapPin className="h-3.5 w-3.5 flex-shrink-0 group-hover:text-primary/70 transition-colors" />
          ...
          {/* Upcoming hearings */}
          {upcomingHearings > 0 && (
            <div className="flex items-center gap-1 mt-2 animate-in fade-in slide-in-from-left duration-500">
        ...
        {/* Arrow */}
        <ChevronRight className="h-5 w-5 text-muted-foreground group-hover:text-primary transition-all group-hover:translate-x-1 flex-shrink-0" />
      </div>
    </div>
  );
}
